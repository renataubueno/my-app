...
export default class Associar extends Component {
  ...
  handleCloseSalvar = () => {
    ...
    for (let i = 0; i < this.state.todasAssociacoes.length; i++) {
      if (
        this.state.todasAssociacoes[i].origem === this.state.valueOrigem &&
        this.state.todasAssociacoes[i].destino === this.state.valueDestino
      ) {
        jaExiste = true;
        break;
      }
    ...
    if (jaExiste) {
      ...
      if (this.state.valueOrigem === "Entrada") {
        let chegada = {
          ...
          destino: this.state.valueDestino // Alterado de chegada pois o valor era sempre 0
        };

        let filaDestino = this.state.filaFilas.filter(
          item => item.id === this.state.valueDestino
        );
        ...
        let filaOrigem = this.state.filaFilas.filter(
          item => item.id === this.state.valueOrigem
        );
        let filaDestino = this.state.filaFilas.filter(
          item => item.id === this.state.valueDestino
        );
  ...
  render() {
    return (
      <div>
        <Button
          variant="contained"
          onClick={this.handleClickOpen} /*className={classes.button}*/
        >
          Associar Filas
        </Button>
        <Dialog
        ...
        >
          ...
          <DialogContent>
            ...
            <Select
              value={this.state.valueOrigem}
              onChange={this.filaSelecionadaOrigem}
            >
              <MenuItem value="">
                <em>Origem</em>
              </MenuItem>
            ...
            <Select
              value={this.state.valueDestino}
              onChange={this.filaSelecionadaDestino}
            >
              <MenuItem value="">
                <em>Destino</em>
              </MenuItem>
