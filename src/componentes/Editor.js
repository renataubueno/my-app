...
class Editor extends React.Component {
  ...
  componentWillMount() {
    ...
    Pubsub.subscribe(
      "retorno-tipo-distribuicao",
      (topico, dadosDaDistribuicao) => {
    ...
    Pubsub.subscribe("deletar-fila", (topico, deletar) => {
      let filaParaDeletar = this.state.filaFilas.filter(
        item => item.id === deletar.id
      );
      ...
      for (let i = 0; i < this.state.filaFilas.length; i++) {
        if (this.state.filaFilas[i].id === deletar.id) {
          console.log(
            "Achei a fila que queria deletar",
            this.state.filaFilas[i]
          );
  ...
  trataFilas = () => {
    return this.state.filaFilas.map(item => (
      <FilaEditor
        objeto={item}
